...
*  Correção semântica (ajuste de links `<a>` inseridos erroneamente dentro de elementos `<button>`).
...
*  Structural semantics correction (fixing `<a>` links erroneously placed inside `<button>` elements).
